...
Now that your robot exists and can walk in a simulation, it needs to understand its environment so it doesn't walk straight into a wall. For this tasks we will use SLAM and AMCL. But it’s not a battle of "SLAM vs. AMCL" **they work together**.
...
#### For Nav2
- `robot_radius`: the size of the robot. So that the legs aren't counted as a wall.
...
**The problem**: the robot would freeze and refuse to move if an obstacle to close. 
<br>
**The fix**: first verify your `robot_radius`. I had to adjust mine to 0.08 (6 cm actual radius + 2 cm safety buffer) in **both the local_costmap and global_costmap**. I also reduced the inflation_radius from 0.7 to 0.2 so narrow doorways would not count as walls.
