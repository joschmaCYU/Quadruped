...
The Problem: The robot would freeze and refuse to move if an obstacle was 45 cm away, even though the physical robot is only 12 cm wide. The MPPI controller was aborting all trajectories.
<br>
The Fix: The default Nav2 my_nav2.yaml sets robot_radius to 0.22 (a 44 cm diameter robot). I had to adjust robot_radius to 0.08 (6 cm actual radius + 2 cm safety buffer) in both the local_costmap and global_costmap. I also drastically reduced the inflation_radius from 0.7 to 0.2 so the planner wouldn't treat narrow doorways as solid walls.
...
<br>
The Fix: This was a dual-layered TF conflict.
1) In my_nav2.yaml, the robot_base_frame was mistakenly set to base_link instead of base_footprint, creating a feedback loop.
2) The SLAM algorithm was ignoring the kinematics because the robot was mathematically constrained. I bypassed this by forcing the IK node to publish the true physical Z-axis gyroscope rotation (/gyro_z_speed) directly to the odometry calculation, completely overriding the theoretical cmd_vel rotation.
