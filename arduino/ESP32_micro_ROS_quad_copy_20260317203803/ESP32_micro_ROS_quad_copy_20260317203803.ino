...
void setup() {
  ...
  // ---------------------------------------------------------
  // 3. ALLOCATE MEMORY FOR SERVO ARRAY (LE CORRECTIF EST ICI)
  // ---------------------------------------------------------
  msg.data.capacity = 8;
...
void loop() {
  switch (state) {
    case WAITING_AGENT:
      // Check if the physical connection to the agent is alive
      if (rmw_uros_ping_agent(100, 1) == RMW_RET_OK) {
        state = AGENT_AVAILABLE;
      } else {
        // Blink while searching for the agent
        digitalWrite(ledPin, HIGH); delay(250);
        digitalWrite(ledPin, LOW); delay(250);
      }
      break;
    ...
    case AGENT_AVAILABLE:
      // Try to create all publishers, subscribers, and timers
      if (create_entities()) {
        state = AGENT_CONNECTED;
        digitalWrite(ledPin, HIGH); // Steady light when connected successfully
      } else {
        state = WAITING_AGENT;
        destroy_entities();
      }
    ...
    case AGENT_CONNECTED:
      // Spin the executor normally. If it returns an error, connection was lost.
    ...
    case AGENT_DISCONNECTED:
      // Clean memory and go back to searching state
      destroy_entities();
      state = WAITING_AGENT;
      break;
